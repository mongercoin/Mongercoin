...
#include "ui_zMongercontroldialog.h"
...
std::list<std::string> ZMongerControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMongerControlDialog::listMints;

ZMongerControlDialog::ZMongerControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMongerControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMongerControlDialog::~ZMongerControlDialog()
{
    delete ui;
}

void ZMongerControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMongerControlDialog::updateList()
{
...
void ZMongerControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMongerControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMonger_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMongerControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMongerControlDialog::GetSelectedMints()
{
...
void ZMongerControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
