...
#include "zMongercontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMGR ought to be enough for anybody." - Bill Gates, 2017
    ui->zMGRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMGRSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMGR->setEnabled(false);
        ui->pushButtonMintzMGR->setToolTip(tr("zMGR is currently disabled due to maintenance."));

        ui->pushButtonSpendzMGR->setEnabled(false);
        ui->pushButtonSpendzMGR->setToolTip(tr("zMGR is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMGRpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMGR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMGR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMGR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMGR in ") +
...
void PrivacyDialog::on_pushButtonSpendzMGR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMGR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMGR
        sendzMGR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMGR
    sendzMGR();
}

void PrivacyDialog::on_pushButtonZMongerControl_clicked()
{
    ZMongerControlDialog* zMongerControl = new ZMongerControlDialog(this);
    zMongerControl->setModel(walletModel);
    zMongerControl->exec();
}

void PrivacyDialog::setZMongerControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMongerSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMGR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Monger Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMGRpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMGR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MGR </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMGR</b>";
    ...
    // use mints from zMonger selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMongerControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMongerControlDialog::GetSelectedMints();
    }
    ...
    // Clear zMonger selector in case it was used
    ZMongerControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMonger Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Monger, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMonger Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMGR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMGR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMGR "));
    ui->labelzMGRAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMGRSyncStatus->setVisible(fShow);
}
